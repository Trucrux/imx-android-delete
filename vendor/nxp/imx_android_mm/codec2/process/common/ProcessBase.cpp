...
void ProcessBase::getGpuPhysicalAddress(int fd, uint32_t size, uint64_t* physAddr)
{
    ...
    if(fd < 0 || size <= 0) {
        PP_BASE_ERR_LOG("%s line %d: invalid fd=%d, size=%d", __FUNCTION__, __LINE__, fd, size);
        return;
    }
