...
//#define LOG_NDEBUG 0
#define LOG_TAG "IsiFilterInterface"
...
// static
const std::string IsiFilterInterface::NAME = "c2.imx.isi.prefil";
// static
const FilterPlugin_V1::Descriptor IsiFilterInterface::DESCRIPTOR = {
    // controlParams
    { 
...
IsiFilterInterface::IsiFilterInterface(C2String name, c2_node_id_t id, const std::shared_ptr<C2ReflectorHelper>& helper)
...
C2R IsiFilterInterface::IntraRefreshSetter(bool mayBlock, C2P<C2StreamIntraRefreshTuning::output> &me) {
    (void)mayBlock;
    C2R res = C2R::Ok();
    if (me.v.period < 1) {
...
C2R IsiFilterInterface::BitrateSetter(bool mayBlock, C2P<C2StreamBitrateInfo::output> &me) {
    (void)mayBlock;
    C2R res = C2R::Ok();
    if (me.v.value <= 4096) {
        me.set().value = 4096;
...
bool IsiFilterInterface::IsFilteringEnabled([[maybe_unused]]
    const std::shared_ptr<C2ComponentInterface> &intf){
    return true;
}
c2_status_t IsiFilterInterface::QueryParamsForPreviousComponent(
