...
Lsc::Lsc(XMLDocument &document) : Abstract(document) { name = NAME_LSC; }

void Lsc::composeAttributes(XMLElement &element) {
  element.SetAttribute(KEY_ENABLE, isEnable);
}

void Lsc::composeSubElements(XMLElement &element) {
  element.SetAttribute(KEY_ADAPTIVE, config.isAdaptive);
}

void Lsc::parseAttributes(XMLElement &element) {
  element.QueryBoolAttribute(KEY_ENABLE, &isEnable);
}

void Lsc::parseSubElements(XMLElement &element) {
