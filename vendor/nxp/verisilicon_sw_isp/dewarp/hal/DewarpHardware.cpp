...
bool DewarpHardware::create(int id) {
...
uint32_t DewarpHardware::getLutStatus() {
    return 0;
}
...
bool DewarpHardware::create(int id) {
    ...
    for (int index = 0; index < 20; index++) {
        ...
        ALOGI("dwe type %s %d ", szDeviceName,  index);
...
void DewarpHardware::setMapLutAddr(uint32_t index, uint64_t addr) {
    ...
    CurLutAddr[index] = addr;
}

uint32_t DewarpHardware::getLutStatus() {
    uint32_t lut_status;
    dwe_ioctl(DWEIOC_GET_LUT_STATUS, &lut_status);
    return lut_status;
}
