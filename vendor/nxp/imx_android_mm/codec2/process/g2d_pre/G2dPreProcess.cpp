...
status_t G2dPreProcess::onProcess() {
    ...
    uint64_t addr_in = inPhys;
    getGpuPhysicalAddress(inFd, sInFormat.bufferSize, &addr_in);
    pPPHandle->sInput.planes[0] = (int)addr_in;
    ALOGV("onProcess IN fd=%d, bufferSize=%u, addr=%lx,inPhys=%lx",inFd,sInFormat.bufferSize, addr_in, inPhys);
    ...
    uint64_t addr_out = outPhys;
    getGpuPhysicalAddress(outFd, sOutFormat.bufferSize, &addr_out);
    pPPHandle->sOutput.planes[0] = (int)addr_out;
    ALOGV("onProcess OUT fd=%d, bufferSize=%u, addr=%lx,outPhys=%lx",outFd,sOutFormat.bufferSize, addr_out, outPhys);
