    ...
    const std::unique_ptr<C2Work> &work, uint8_t **ppOutputBuffer, uint32_t *pOutputSize)
{
    ...
    *pOutputSize = 0;
    ...
    } else if (C2_OK == mUtil->parseFrame(pInputBuffer,InputLen,&FrameInfo)
                    && FrameInfo.bGotOneFrame && FrameInfo.nFrameSize > 0) {
        uint32_t nActualInLen = 0;
        InputLen = FrameInfo.nFrameSize;
        InputOffset = 0;
        AudioRingBuffer.BufferGet(&pInputBuffer, InputLen, &nActualInLen);
        if (nActualInLen < InputLen) {
            ALOGW("Can't get a full frame(%d) from ringbuffer, nActualLen=%d",InputLen, nActualInLen);
    ...
    }else if(decoderRet == ACODEC_NOT_ENOUGH_DATA || decoderRet == ACODEC_NO_OUTPUT){
        ret = AUDIO_DECODE_NEEDMORE;
...
c2_status_t UniaDecoder::checkFrameHeader()
{
    ...
    nPushModeInputLen = mUtil->getPushModeInputLen();
    LOGI("update nPushModeInputLen: %d", nPushModeInputLen);
    return ret;
}
