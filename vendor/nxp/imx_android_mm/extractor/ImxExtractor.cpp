...
codec_mime_struct video_mime_table[]={
    ...
    {VIDEO_DOLBY_VISION,0,MEDIA_MIMETYPE_VIDEO_DOLBY_VISION},
};
...
size_t ImxExtractor::countTracks()
{
    ...
    if (!strcmp(mMime, MEDIA_MIMETYPE_CONTAINER_MPEG4))
        return mNumTracks; // CTS MediaMetadataRetrieverTest requires track number including unsupported tracks
    else
        return mTracks.size();
}
...
media_status_t ImxExtractor::getTrackMetaData(AMediaFormat *meta,size_t index, uint32_t flags)
{
    ...
    if(index >= mTracks.size()){
        // set default value for unsupported track
        AMediaFormat_setString(meta, AMEDIAFORMAT_KEY_MIME, "application/octet-stream");
        return AMEDIA_OK;
    }
    else
...
status_t ImxExtractor::ParseVideo(uint32 index, uint32 type,uint32 subtype)
{
    ...
    if(decoderSpecificInfoSize > 0 && decoderSpecificInfo != NULL){
        ...
        if(type == VIDEO_H264){
            ...
            bool isoHEVC = true;
            if (!strcmp(mMime, MEDIA_MIMETYPE_CONTAINER_ASF))
                isoHEVC = false;

            //HEVCDecoderConfigurationRecord in 14496-15:8.3.3.1
            if(isoHEVC || isISOHevcCodecPrivate(decoderSpecificInfo, decoderSpecificInfoSize)){
                ...
                ALOGI("add KEY_CSD_HEVC metadata for hevc video size=%u",decoderSpecificInfoSize);
            }else{
                //some container did not follow 14496-15:8.3.3.1 HEVCDecoderConfigurationRecord,
                //set nal start code and nals into AMEDIAFORMAT_KEY_CSD_0
                addASFHevcCodecPrivate(meta, decoderSpecificInfo, decoderSpecificInfoSize);
            }
        ...
        }else if(type == VIDEO_DOLBY_VISION){
            // content of info:  |csd0Size|csd2Size|csd_hevcSize|csd_avcSize|csd0|csd2|csd_hevc|csd_avc
            uint32 csd0Size, csd2Size, csd_hevcSize, csd_avcSize;
            uint8 * data = decoderSpecificInfo;
            do{
                if(decoderSpecificInfoSize < 16){
                    ALOGE("csd size too small!");
                    break;
                }
                csd0Size = *(uint32 *)data;
                data += 4;
                csd2Size = *(uint32 *)data;
                data += 4;
                csd_hevcSize = *(uint32 *)data;
                data += 4;
                csd_avcSize = *(uint32 *)data;
                data += 4;
                ALOGV("csd0 %d, csd2 %d, csd_hevc %d, csd_avc %d", csd0Size , csd2Size, csd_hevcSize, csd_avcSize);
                if(csd0Size + csd2Size + csd_hevcSize + csd_avcSize + 16 > decoderSpecificInfoSize){
                    ALOGE("csd size mismatch!");
                    break;
                }
                if(csd0Size > 0){
                    AMediaFormat_setBuffer(meta, AMEDIAFORMAT_KEY_CSD_0, data, csd0Size);
                    ALOGI("add CSD_0 metadata size=%u", csd0Size);
                }
                data += csd0Size;
                if(csd2Size > 0){
                    AMediaFormat_setBuffer(meta, AMEDIAFORMAT_KEY_CSD_2, data, csd2Size);
                    ALOGI("add CSD_2 metadata size=%u", csd2Size);
                }
                if(csd2Size >= 5){
                        profile = data[2] >> 1;
                        const uint8_t blCompatibilityId = (data[4]) >> 4;

                        if (blCompatibilityId && blCompatibilityId != 15) {
                            ALOGI("create_two_tracks with profile %d", profile);
                            create_two_tracks = true;
                        }
                }
                data += csd2Size;
                if(csd_hevcSize > 0){
                    AMediaFormat_setBuffer(meta, AMEDIAFORMAT_KEY_CSD_HEVC, data, csd_hevcSize);
                    ALOGI("add hvcc metadata size=%u",csd_hevcSize);
                }
                data += csd_hevcSize;
                if(csd_avcSize > 0){
                    AMediaFormat_setBuffer(meta, AMEDIAFORMAT_KEY_CSD_AVC, data, csd_avcSize);
                    ALOGI("add avcc metadata size=%u",csd_avcSize);
                }
            }while(0);
        }else{
    ...
    if(create_two_tracks){
        mTracks.push();
        sourceIndex = mTracks.size() - 1;
        TrackInfo *trackInfo2 = &mTracks.editItemAt(sourceIndex);
        memcpy(trackInfo2, trackInfo, sizeof(TrackInfo));
        trackInfo2->mMeta = AMediaFormat_new();
        AMediaFormat_copy(trackInfo2->mMeta, trackInfo->mMeta);
        uint8_t emptybuffer[8] = {};
        AMediaFormat_setBuffer(trackInfo2->mMeta, AMEDIAFORMAT_KEY_CSD_2, emptybuffer, 0);
        if (4 == profile || 7 == profile || 8 == profile ) {
            AMediaFormat_setString(trackInfo2->mMeta, AMEDIAFORMAT_KEY_MIME, MEDIA_MIMETYPE_VIDEO_HEVC);
        } else if (9 == profile) {
            AMediaFormat_setString(trackInfo2->mMeta, AMEDIAFORMAT_KEY_MIME, MEDIA_MIMETYPE_VIDEO_AVC);
        } else if (10 == profile) {
            AMediaFormat_setString(trackInfo2->mMeta, AMEDIAFORMAT_KEY_MIME, MEDIA_MIMETYPE_VIDEO_AV1);
        }

        mNumTracks++;
        const char *mime;
        AMediaFormat_getString(trackInfo2->mMeta, AMEDIAFORMAT_KEY_MIME, &mime);
        ALOGI("add backward compatible track index=%u,source index=%zu,mime=%s",index,sourceIndex,mime);
    }
...
status_t ImxExtractor::ParseAudio(uint32 index, uint32 type,uint32 subtype)
{
    ...
    if(bitPerSample > 0 ){
        if(type == AUDIO_WMA)
            AMediaFormat_setInt32(meta, AMEDIAFORMAT_VENDOR_KEY_BITS_PER_SAMPLE, bitPerSample);
        else if (!strcmp(mMime, MEDIA_MIMETYPE_CONTAINER_MPEG4)){
            if(type == AUDIO_PCM || type == AUDIO_ALAC)
...
bool ImxExtractor::isISOHevcCodecPrivate(uint8_t *priv, size_t privSize)
{
    ...
    if((priv[15] & 0xFC) != 0xFC)
        return false;

    if((priv[16] & 0xFC) != 0xFC)
        return false;

    if((priv[17] & 0xF8) != 0xF8)
        return false;

    if((priv[18] & 0xF8) != 0xF8)
        return false;

    uint8_t i, j;
    uint8_t *data = priv + 23;
    uint8_t type;
    uint16_t numNalus;
    int32_t size = privSize - 23;
    uint16_t nalLength;
    uint8_t numOfArrays = priv[22];

    if(numOfArrays < 3)
        return false;
    ALOGV("isISOHevcCodecPrivate numOfArrays=%d",numOfArrays);
    ...
    for (i = 0; i < numOfArrays; i++) {
        ...
        // Num of nals
        numNalus = (data[0] << 8) + data[1];
        data += 2;
        size -= 2;
        ALOGV("isISOHevcCodecPrivate numOfArrays=%d, type =%d, nalNum=%d",i,type,numNalus);
...
void ImxExtractor::addASFHevcCodecPrivate(AMediaFormat *meta, uint8_t *priv, size_t privSize)
{
    if(privSize > 4 && priv[0] == 0x0 && priv[1] == 0x0 
        && priv[2] == 0x0 && priv[3] == 0x1){
        //HEVC_1280x720_25fps_1201Kbps_AAC_44.1Khz_2ch_36Kbps.WMV
        AMediaFormat_setBuffer(meta, AMEDIAFORMAT_KEY_CSD_0, priv, privSize);
        ALOGI("add KEY_CSD_0 start_code metadata for hevc video size=%zu",privSize);
        return;
    }

    //HEVC_640x480_23.976fps_736Kbps_AC3_48Khz_192Kbps_Successful-Mission.asf
    int32_t size = static_cast<int32_t>(privSize);
    uint8_t *data = priv;
    uint16_t NALLength = 0;
    uint32 info_size = 0;

    //get target codec data len for start code
    while(size > 0){
        if (size <= 2)
            break;
    ...
    uint8_t *new_data = new uint8_t[info_size];
    uint8_t *new_data_ptr = new_data;

    size = static_cast<int32_t>(privSize);
    data = priv;
    ...
    while(size > 0){
        ...
        *(new_data_ptr+3)=0x01;
        ...
        //as start code needs 4 bytes, move 2 bytes for start code,
        //the other 2 bytes uses the nal length
        memcpy(new_data_ptr+4, data, NALLength);

        new_data_ptr += 4+ NALLength;

        data += NALLength;
        size -= NALLength;
    }

    AMediaFormat_setBuffer(meta, AMEDIAFORMAT_KEY_CSD_0, new_data, info_size);
    delete[] new_data;
    new_data = NULL;

    ALOGI("add KEY_CSD_0 nal_length metadata for hevc video size=%zu",privSize);
    return;
}
